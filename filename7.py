...
timeout = 5        # Timeout in seconds (if necessary)
    ...
    # Create the Modbus client and connect
#client = ModbusClient(ip_address, port=port, timeout=timeout)
import cv2

# Pipeline GStreamer
pipeline = (
    "gst-launch-1.0 v4l2src device=/dev/video0 ! image/jpeg, width=1920, height=1080, framerate=60/1, format=MJPG ! nvv4l2decoder mjpeg=1 ! nvvidconv ! videocrop name=cropper  ! xvimagesink sync=False"
)
...
class camera1(QThread):
    ...
    def run(self):
        ...
        cap = nano.Camera(camera_type =1,device_id=1,width= 1920,height= 1080, fps= 30)
        while True:
            ...
            if cap.isReady():           
                ...
                for r in range(globalvar.qtycam1):
                    ...
                    # if client.connect():
                    #         # Kết nối thành công, có thể thực hiện các thao tác Modbus
                    #         # Đọc các biến Y,M,...FX5U
                    #
                    #         #------------------------
                    #     result_Y0 = client.read_coils(0,1)  # Địa chỉ Modbus 00001 tương ứng với địa chỉ 0 trong Python
                    #     client.write_coil(1, True) #Write Y1 On FX5U
                    #     if result_Y0.isError():
                    #         print("Error reading coil")
                    #     else:
                    #         coil_value_Y0 = result_Y0.bits[0]  # Lấy trạng thái của coil tại địa chỉ 0
                    #         print(f'Trạng thái của coil 00001: {coil_value_Y0}')
                    #
                    #         # Đọc/ghi các thanh ghi D0,D1 FX5U
                    #
                    #         #----------------------------------
                    #     result_D0 = client.read_holding_registers(0, 1)
                    #         #nạp giá trị 1234 vào D1
                    #     client.write_register(0, 1234)
                    #     if not result_D0.isError():
                    #         D0 = result_D0.registers[0]
                    #         print(result_D0.registers[0])  # In giá trị từ thanh ghi
                    #     else:
                    #         print("Error reading holding register")
                    #         # Đọc giá trị từ Input Register tại địa chỉ Modbus 30001 (Python index 0)
                    #
                    #         #----------------------------
                    #     result_RD0 = client.read_input_registers(0, 1)
                    #         # Kiểm tra xem có lỗi không
                    #     if not result_RD0.isError():
                    #             # Lấy giá trị từ register đầu tiên (và duy nhất) đã đọc
                    #         input_value_RD0 = result_RD0.registers[0]
                    #         print(f'Giá trị của Input Register 30001: {input_value_RD0}')
                    #     else:
                    #         print("Lỗi khi đọc Input Register")
                    #         # Đọc trạng thái từ Discrete Input tại địa chỉ Modbus 10001 (Python index 0)
                    #
                    #         #--------------------------------------------------------------
                    #     result_X0 = client.read_discrete_inputs(0, 1)
                    #         # Kiểm tra xem có lỗi không
                    #     if not result_X0.isError():
                    #             # Lấy trạng thái của input đầu tiên (và duy nhất) đã đọc
                    #         input_status = result_X0.bits[0]
                    #         print(f'Trạng thái của Discrete Input 10001: {input_status_X0}')
                    #     else:
                    #         print("Lỗi khi đọc Discrete Input")
                    # else:
                    #     print(f"Không thể kết nối đến thiết bị tại IP {ip_address}")
