...
class camera1(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            M200_bit = client.read_coils(200, 1)#Mo camera
            M201_bit = client.read_coils(201, 1)# Kiem tra camera
            client.write_register(300, 0)# out tin hieeu OK vai
            client.write_register(301, 0)# out ti hieu NG vai
            if M200_bit.isError():
                print("Error reading coil")
            else:
                # coil_value_Y0 = result_Y0.bits[0]  # Lấy trạng thái của coil tại địa chỉ 0
                # 3print(f'Trạng thái của coil 00001: {coil_value_Y0}')
            ...
            if M200 == 1 and cap.isReady():
                ...
                for r in range(globalvar.qtycam1):
                    ...
                    height, width, channels = shapes_cap_out.shape
                    ...
                    xcenter, ycenter = int(globalvar.read_database[r, 21]), int(globalvar.read_database[r, 22])
                    r1, r2 = int(globalvar.read_database[r, 24]), int(globalvar.read_database[r, 25])
                    cv2.ellipse(mask_outside, (xcenter, ycenter), (r1, r1), globalvar.read_database[r, 23], 0,
                                360, (255, 255, 255), -1)
                    cv2.ellipse(mask_inside, (xcenter, ycenter), (r2, r2), globalvar.read_database[r, 26], 0,
                                360,
                                (255, 255, 255), -1)
                    #print("x",xcenter)
                    #print("y", ycenter)
                    # taoj ring mau trang
                    mask_ring = cv2.bitwise_xor(mask_inside, mask_outside)
                    #image_home = cv2.bitwise_and(mask_outside, mask_ring)
                    img_mask1 = self.cap_out[0:height,0:width]
                    #inside = cv2.bitwise_and(imgCropped_1, imgCropped_1, mask=inside_inv)

                    image_home = cv2.bitwise_and(img_mask1 ,img_mask1,mask= mask_ring)
                    ...
                    imgCropped = image_home[globalvar.read_database[r, 4]:globalvar.read_database[r, 5],
                                 globalvar.read_database[r, 6]:globalvar.read_database[r, 7]]
                    ...
                    if os.path.exists(file_path.format(globalvar.pictureid)):
                        ...
                        print("hay luu anh")
                    #imgstack = self.stackImages(1,([imgCropped,imgBinary],[mask_outside,mask_inside]))
                ...
                #self.img[mask_out] = cv2.addWeighted(self.cap_out,alpha,shapes_cap_out ,1-alpha,0)[mask_out]image_home
                self.img[mask_out] = cv2.addWeighted(image_home, alpha, shapes_cap_out, 1 - alpha, 0)[mask_out]
