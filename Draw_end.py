...
class ImageProcessor:
    def __init__(self, camera_type='CSI', camera_source=0):
        # Biến toàn cục
        self.shapes = []
        self.drawing = False
        self.dragging = False
        ...
        # Mở camera dựa trên loại camera (CSI hoặc USB)
        self.camera_type = camera_type
        if self.camera_type == 'CSI':
            # Sử dụng nanocamera cho camera CSI
            self.camera = nano.Camera(flip=0, width=640, height=480, fps=30, camera_type=nano.CAMERA_CSI)
        elif self.camera_type == 'USB':
            # Sử dụng OpenCV cho camera USB
            self.camera = cv2.VideoCapture(camera_source)
            if not self.camera.isOpened():
                raise Exception(f"Không thể mở camera từ nguồn {camera_source}.")
        else:
            raise ValueError("Loại camera không hợp lệ. Chọn 'CSI' hoặc 'USB'.")
...
def main():
    # Sử dụng camera CSI hoặc USB
    processor = ImageProcessor(camera_type='USB')  # Thay 'CSI' bằng 'USB' nếu sử dụng camera USB
    cropped_images = []
    ...
    if processor.camera_type == 'CSI':
        processor.camera.release()  # Giải phóng camera CSI
    elif processor.camera_type == 'USB':
        processor.camera.release()  # Giải phóng camera USB
